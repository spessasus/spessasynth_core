...
const builder = new MIDIBuilder({
    name: "GS Reverb Pan Delay Feedback Test"
});
...
while (feedback <= 128) {
    // Feedback
    sendAddress(0x40, 0x01, 0x35, Math.min(127, feedback));
    ticks += 480;
...
builder.flush();
void fs.writeFile(
    "files/test_gs__reverb_pan_delay_feedback.mid",

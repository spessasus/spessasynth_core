...
import { readRIFFChunk } from "../soundbank/basic_soundbank/riff_chunk.js";
import { readVariableLengthQuantity } from "../utils/byte_functions/variable_length_quantity.js";
import { readBytesAsUintBigEndian } from "../utils/byte_functions/big_endian.js";
import { readBytesAsString } from "../utils/byte_functions/string.js";
import { readLittleEndian } from "../utils/byte_functions/little_endian.js";
import { type messageTypes, RMIDINFOChunks } from "./enums.ts";
import { BasicMIDI } from "./basic_midi.js";
import { loadXMF } from "./xmf_loader.js";
import type { MIDIFormat } from "./types.ts";
...
class MIDI extends BasicMIDI {
    ...
    constructor(arrayBuffer: ArrayBuffer, fileName: string = "") {
            ...
            while (binaryData.currentIndex <= binaryData.length) {
                ...
                if (currentChunk.header === "RIFF") {
                    ...
                    if (type === "sfbk" || type === "sfpk" || type === "dls ") {
                        ...
                        this.embeddedSoundFont = binaryData.slice(
                            startIndex,
                    ...
                    if (type === "INFO") {
                        ...
                        this.RMIDInfo = {};
                        while (
                            currentChunk.chunkData.currentIndex <=
                            currentChunk.size
                        ) {
                            ...
                            this.RMIDInfo[infoChunk.header as RMIDINFOChunks] =
                                infoChunk.chunkData;
                        }
                        ...
                        if (this.RMIDInfo["INAM"]) {
                            this.rawMidiName = this.RMIDInfo[
                                RMIDINFOChunks.name
                            ] as IndexedByteArray;
                        ...
                        if (this.RMIDInfo[RMIDINFOChunks.bankOffset]) {
                            this.bankOffset = readLittleEndian(
                                this.RMIDInfo[RMIDINFOChunks.bankOffset],
                                2
                            );
                        }
                    }
        ...
        // format
        this.format = readBytesAsUintBigEndian(
            headerChunk.data,
            2
        ) as MIDIFormat;
        // tracks count
        ...
        for (let i = 0; i < this.tracksAmount; i++) {
            ...
            if (this.format === 2 && i > 0) {
                totalTicks +=
                    this.tracks[i - 1][this.tracks[i - 1].length - 1].ticks;
            }
                ...
                switch (statusByteChannel) {
                    ...
                    default:
                        ...
                        eventDataLength =
                            dataBytesAmount[
                                (statusByte >>
                                    4) as keyof typeof dataBytesAmount
                            ];
